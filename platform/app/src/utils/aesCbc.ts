/**
 * AES-CBC Encryption/Decryption Utility
 *
 * Uses native Web Crypto API (crypto.subtle) in secure contexts (HTTPS/localhost).
 * Falls back to pure-JS AES-CBC implementation for non-secure contexts (HTTP).
 */

// ─── Pure JS AES-CBC Implementation (fallback) ───────────────────────────

const SBOX = new Uint8Array([
  ,
]);

const INV_SBOX = new Uint8Array([
  ,
]);

const RCON = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];

/** GF(2^8) multiplication */
function gmul(a: number, b: number): number {
  let p = 0;
  for (let i = 0; i < 8; i++) {
    if (b & 1) p ^= a;
    const hi = a & 0x80;
    a = (a << 1) & 0xff;
    if (hi) a ^= 0x1b;
    b >>= 1;
  }
  return p;
}

/** AES-128 key expansion → 11 round keys (176 bytes) */
function expandKey(key: Uint8Array): Uint8Array {
  const expanded = new Uint8Array(176);
  expanded.set(key.subarray(0, 16));

  let bytesGenerated = 16;
  let rconIndex = 0;

  while (bytesGenerated < 176) {
    let tmp = expanded.slice(bytesGenerated - 4, bytesGenerated);

    if (bytesGenerated % 16 === 0) {
      // RotWord + SubWord + Rcon
      const t = tmp[0];
      tmp[0] = SBOX[tmp[1]] ^ RCON[rconIndex++];
      tmp[1] = SBOX[tmp[2]];
      tmp[2] = SBOX[tmp[3]];
      tmp[3] = SBOX[t];
    }

    for (let i = 0; i < 4; i++) {
      expanded[bytesGenerated] = expanded[bytesGenerated - 16] ^ tmp[i];
      bytesGenerated++;
    }
  }

  return expanded;
}

/** Encrypt a single 16-byte block */
function encryptBlock(block: Uint8Array, expandedKey: Uint8Array): Uint8Array {
  const state = new Uint8Array(16);
  state.set(block);

  // AddRoundKey (round 0)
  for (let i = 0; i < 16; i++) state[i] ^= expandedKey[i];

  for (let round = 1; round <= 10; round++) {
    // SubBytes
    for (let i = 0; i < 16; i++) state[i] = SBOX[state[i]];

    // ShiftRows (column-major: state[row + 4*col])
    // Row 1: shift left 1
    let t = state[1];
    state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = t;
    // Row 2: shift left 2
    t = state[2]; state[2] = state[10]; state[10] = t;
    t = state[6]; state[6] = state[14]; state[14] = t;
    // Row 3: shift left 3 (= right 1)
    t = state[15];
    state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = t;

    // MixColumns (skip in last round)
    if (round < 10) {
      for (let c = 0; c < 4; c++) {
        const i = c * 4;
        const s0 = state[i], s1 = state[i+1], s2 = state[i+2], s3 = state[i+3];
        state[i]   = gmul(2,s0) ^ gmul(3,s1) ^ s2        ^ s3;
        state[i+1] = s0         ^ gmul(2,s1) ^ gmul(3,s2) ^ s3;
        state[i+2] = s0         ^ s1         ^ gmul(2,s2) ^ gmul(3,s3);
        state[i+3] = gmul(3,s0) ^ s1         ^ s2         ^ gmul(2,s3);
      }
    }

    // AddRoundKey
    const rkOffset = round * 16;
    for (let i = 0; i < 16; i++) state[i] ^= expandedKey[rkOffset + i];
  }

  return state;
}

/** Decrypt a single 16-byte block */
function decryptBlock(block: Uint8Array, expandedKey: Uint8Array): Uint8Array {
  const state = new Uint8Array(16);
  state.set(block);

  // AddRoundKey (round 10)
  for (let i = 0; i < 16; i++) state[i] ^= expandedKey[160 + i];

  for (let round = 9; round >= 0; round--) {
    // InvShiftRows
    // Row 1: shift right 1
    let t = state[13];
    state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = t;
    // Row 2: shift right 2
    t = state[2]; state[2] = state[10]; state[10] = t;
    t = state[6]; state[6] = state[14]; state[14] = t;
    // Row 3: shift right 3 (= left 1)
    t = state[3];
    state[3] = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = t;

    // InvSubBytes
    for (let i = 0; i < 16; i++) state[i] = INV_SBOX[state[i]];

    // AddRoundKey
    const rkOffset = round * 16;
    for (let i = 0; i < 16; i++) state[i] ^= expandedKey[rkOffset + i];

    // InvMixColumns (skip in round 0)
    if (round > 0) {
      for (let c = 0; c < 4; c++) {
        const i = c * 4;
        const s0 = state[i], s1 = state[i+1], s2 = state[i+2], s3 = state[i+3];
        state[i]   = gmul(14,s0) ^ gmul(11,s1) ^ gmul(13,s2) ^ gmul(9,s3);
        state[i+1] = gmul(9,s0)  ^ gmul(14,s1) ^ gmul(11,s2) ^ gmul(13,s3);
        state[i+2] = gmul(13,s0) ^ gmul(9,s1)  ^ gmul(14,s2) ^ gmul(11,s3);
        state[i+3] = gmul(11,s0) ^ gmul(13,s1) ^ gmul(9,s2)  ^ gmul(14,s3);
      }
    }
  }

  return state;
}

/** PKCS7 padding */
function pkcs7Pad(data: Uint8Array): Uint8Array {
  const padLen = 16 - (data.length % 16);
  const padded = new Uint8Array(data.length + padLen);
  padded.set(data);
  for (let i = data.length; i < padded.length; i++) {
    padded[i] = padLen;
  }
  return padded;
}

/** PKCS7 unpadding */
function pkcs7Unpad(data: Uint8Array): Uint8Array {
  const padLen = data[data.length - 1];
  if (padLen < 1 || padLen > 16) {
    throw new Error('Invalid PKCS7 padding');
  }
  for (let i = data.length - padLen; i < data.length; i++) {
    if (data[i] !== padLen) {
      throw new Error('Invalid PKCS7 padding');
    }
  }
  return data.subarray(0, data.length - padLen);
}

/** Pure JS AES-128-CBC encrypt */
function jsAesCbcEncrypt(plaintext: Uint8Array, key: Uint8Array, iv: Uint8Array): Uint8Array {
  const expandedKey = expandKey(key);
  const padded = pkcs7Pad(plaintext);
  const output = new Uint8Array(padded.length);
  let prev = iv;

  for (let offset = 0; offset < padded.length; offset += 16) {
    const block = new Uint8Array(16);
    for (let i = 0; i < 16; i++) {
      block[i] = padded[offset + i] ^ prev[i];
    }
    const encrypted = encryptBlock(block, expandedKey);
    output.set(encrypted, offset);
    prev = encrypted;
  }

  return output;
}

/** Pure JS AES-128-CBC decrypt */
function jsAesCbcDecrypt(ciphertext: Uint8Array, key: Uint8Array, iv: Uint8Array): Uint8Array {
  if (ciphertext.length % 16 !== 0) {
    throw new Error('Ciphertext length must be a multiple of 16');
  }

  const expandedKey = expandKey(key);
  const output = new Uint8Array(ciphertext.length);
  let prev = iv;

  for (let offset = 0; offset < ciphertext.length; offset += 16) {
    const block = ciphertext.subarray(offset, offset + 16);
    const decrypted = decryptBlock(block, expandedKey);
    for (let i = 0; i < 16; i++) {
      output[offset + i] = decrypted[i] ^ prev[i];
    }
    prev = block;
  }

  return pkcs7Unpad(output);
}

// ─── Public API ───────────────────────────────────────────────────────────

const isSubtleAvailable = typeof crypto !== 'undefined' && typeof crypto.subtle !== 'undefined';

/**
 * AES-128-CBC encrypt.
 * Automatically uses native Web Crypto API when available, falls back to pure JS.
 */
export async function encryptAesCbc(
  plaintext: Uint8Array,
  key: Uint8Array,
  iv: Uint8Array
): Promise<ArrayBuffer> {
  if (isSubtleAvailable) {
    const cryptoKey = await crypto.subtle.importKey(
      'raw',
      key,
      { name: 'AES-CBC', length: 128 },
      false,
      ['encrypt']
    );
    return crypto.subtle.encrypt({ name: 'AES-CBC', iv }, cryptoKey, plaintext);
  }

  // Fallback: pure JS
  console.warn('[aesCbc] crypto.subtle unavailable (non-secure context). Using JS fallback.');
  const result = jsAesCbcEncrypt(plaintext, key, iv);
  return result.buffer.slice(result.byteOffset, result.byteOffset + result.byteLength);
}

/**
 * AES-128-CBC decrypt.
 * Automatically uses native Web Crypto API when available, falls back to pure JS.
 */
export async function decryptAesCbc(
  ciphertext: Uint8Array,
  key: Uint8Array,
  iv: Uint8Array
): Promise<ArrayBuffer> {
  if (isSubtleAvailable) {
    const cryptoKey = await crypto.subtle.importKey(
      'raw',
      key,
      { name: 'AES-CBC', length: 128 },
      false,
      ['decrypt']
    );
    return crypto.subtle.decrypt({ name: 'AES-CBC', iv }, cryptoKey, ciphertext);
  }

  // Fallback: pure JS
  console.warn('[aesCbc] crypto.subtle unavailable (non-secure context). Using JS fallback.');
  const result = jsAesCbcDecrypt(ciphertext, key, iv);
  return result.buffer.slice(result.byteOffset, result.byteOffset + result.byteLength);
}
